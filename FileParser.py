...
def _closest_time_index(
    ...
    if not valid_indices:
        raise ValueError(
            "Нет валидных временных колонок"
        )
    diffs = [
        abs(to_minutes(times[i]) - target)
        for i in valid_indices
    ]
    best_local_idx = diffs.index(min(diffs))
    return valid_indices[best_local_idx]

# Извлекает из файла  metadata (dict), spectrum (dict: wavelength -> value)
def _parse_radcalnet_file(
        path,
        column_index
    ):
    metadata = {}
...
def read_radcalnet_by_date(
    ...
    if sample_line is None:
        raise ValueError("Не найдена строка 400nm")
    col_idx = _closest_time_index(times, target_utc_time, sample_line)
